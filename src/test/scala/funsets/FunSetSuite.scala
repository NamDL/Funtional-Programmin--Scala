...
class FunSetSuite extends FunSuite {
  ...
  test("Intersection contains common elements in both the sets set") {
    new TestSets {
      val s = intersect(union(s1, s2), union(s1, s2))
  ...
  test("Difference should contin elements in one set not present in the other"){
    new TestSets{
  ...
  test("Test filter to return subset of set S, that satify the given predicate"){
    new TestSets{
      val s=union(union(s1,s2),union(s3,s4))
  ...
  test("Returns whether there exists a bounded integer within `s` that satisfies `p`"){
    new TestSets{
      ...
      assert(!(exists(s,x=>(x*x)==5)),"exists 2")
    }
  }
